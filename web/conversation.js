import { openImportSync } from "./import-sync.js";
...
export async function renderConversation(ctx) {
  ...
  function paintControls() {
    ...
    for (const run of [...cache.runs.values()].sort((a, b) => b.createdAt.localeCompare(a.createdAt))) if (!latestByBot.has(run.bot)) latestByBot.set(run.bot, run);
    const latest = [...latestByBot.values()].find(run => run.status === "failed" && /Task reached the \d+-(?:step|call tool) budget/.test(run.error || ""));
    const continuation = root.querySelector("[data-continue-task]");
    continuation.hidden = busy || latest?.status !== "failed" || !/Task reached the \d+-(?:step|call tool) budget/.test(latest?.error || "");
    root.querySelector("[data-continue-button]").dataset.runId = latest?.id || "";
  ...
  root.querySelector("[data-continue-button]").onclick = async (event) => {
    const button = event.currentTarget, id = button.dataset.runId;
    button.disabled = true;
    try {
      const result = await post(`/api/runs/${enc(id)}/continue`);
      cache.runVersion++; result.runs.forEach(run => cache.runs.set(run.id, run));
      if (ctx.current()) { paintControls(); void refreshRuns(); } ctx.changed();
  ...
  root.querySelector("[data-sync-import]")?.addEventListener("click", () => { root.querySelector(".conversation-menu").open = false; void openImportSync(ctx, scope); });
