...
test('imports Hermes context and a Grok group into working Linubot conversations', async () => {
  ...
  try {
    ...
    await expect(page.locator('[name=sourceId]').first()).toBeEnabled();
    ...
    await expect(page.locator('[data-selected-count]')).toHaveText('4 selected');
    ...
    await page.getByRole('button', { name: 'Import into Linubot', exact: true }).click();
    await expect(page.getByRole('heading', { name: 'Your teammates are ready' })).toBeVisible();
    await page.getByRole('link', { name: 'Open your bots', exact: true }).click();
    const imported = await page.evaluate(async () => ({ bots: await fetch('/api/bots').then(r => r.json()), groups: await fetch('/api/groups').then(r => r.json()) }));
    expect(imported.bots.map(bot => bot.name).sort()).toEqual(['Fern', 'ImportedHelper', 'Pip']);
    expect(imported.groups).toHaveLength(1);
    await page.evaluate(id => { location.hash = '#/group/' + id; }, imported.groups[0].id);
