...
test('imports Hermes context and a Grok group into working Linubot conversations', async () => {
  ...
  try {
    ...
    const refreshedDb = new DatabaseSync(join(hermes, 'state.db'));
    refreshedDb.exec("INSERT INTO messages VALUES (3,'one','assistant','New imported source message',1003)"); refreshedDb.close();
    write(join(hermes, 'memories/MEMORY.md'), 'The project codename is DESKTOP_IMPORT_7391.\nNEW_SYNC_KNOWLEDGE');
    await page.getByLabel('Bot options', {exact:true}).click();
    await page.getByRole('button', {name:'Sync latest from source',exact:true}).click();
    await expect(page.getByRole('dialog')).toContainText('1 new historical messages');
    await page.getByRole('button', {name:'Sync now',exact:true}).click();
    await expect(page.getByRole('heading', {name:'Sync complete',exact:true})).toBeVisible();
    await page.getByRole('button', {name:'Back to conversation',exact:true}).click();
    await expect(page.locator('.message-body').getByText('New imported source message', {exact:true})).toBeVisible();
    const synced = await page.evaluate(() => fetch('/api/bots/ImportedHelper').then(r=>r.json()));
    expect(synced.importedContext).toContain('NEW_SYNC_KNOWLEDGE');
    await page.getByLabel('Bot options', {exact:true}).click();
    await page.getByRole('button', {name:'Sync latest from source',exact:true}).click();
    await expect(page.getByRole('dialog')).toContainText('0 new historical messages');
