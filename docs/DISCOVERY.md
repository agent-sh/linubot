...
Built-in workspace, memory, session, skills, web, and artifact tools are always
loaded when enabled and permitted for the task. Connected MCP tools start with a
compact catalog of names and load their schemas on demand. Full skill
instructions are loaded separately when needed.

- Built-in tools stay loaded and do not use any MCP recency slots.
...
- `search_tools` discovers connected MCP tool names and descriptions. It loads
  up to five matching MCP tool schemas for the next model call; built-in tools
  are already available.

The runtime retains at most twelve recently searched/used MCP tool schemas.
...
initial names-only MCP tool catalog is bounded; searches can find tools beyond that
