...
import { verifyArtifactPrivacy } from './artifact-privacy.mjs';
...
for (const path of ['resources/app.asar', 'resources/workspace/agent-workspace-linux', 'resources/runners/uv', 'resources/runners/uvx']) {
  const provenance = path === 'resources/workspace/agent-workspace-linux' ? JSON.parse(readFileSync('release/linux-unpacked/resources/workspace/provenance.json', 'utf8')) : undefined;
  verifyArtifactPrivacy(path, readFileSync(`release/linux-unpacked/${path}`), provenance);
}
